...
unmatched_bakes_results
```

```{r}

# Clean results data
results_clean <- read_csv("results.csv", skip = 2) %>%
  rename(Series = `...1`, Episode = `...2`, Baker = `...3`, Technical_Rank = `...4`, Result = `...5`)

# Merge bakers and bakes data
bakes_bakers <- merge(bakes, bakers, by.x = c("Series", "Baker"), by.y = c("Series", "Baker Name"))

# Merge the combined data with results
combined_data <- merge(bakes_bakers, results_clean, by = c("Series", "Episode", "Baker"))

# Organize columns
combined_data <- combined_data %>%
  select(Series, Episode, Baker, `Baker Age`, `Baker Occupation`, `Hometown`, `Signature Bake`, `Show Stopper`, Technical_Rank, Result)

# Export final dataset
write_csv(combined_data, "combined_bakeoff_data.csv")
```
